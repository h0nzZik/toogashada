...
template <typename T> void append(Message &m, T const &object) {
...
template <typename T> void append(Message &m, std::vector<T> const &vec) {
...
public:
  ...
  bool empty() const { return offset == m.data.size(); }
...
template <typename T> struct Take {
  static T from(Deserializer &d) { return d.take<T>(); }
};
...
template <typename T> struct Take<std::vector<T>> {
