...
template <typename Entity, typename... Comps>
struct impl_removeComponent;

template < typename Component, typename... Comps >
struct impl_getComponentNumber;
...
public:
  ...
  static void removeComponent(entity_t entity, uint32_t component_id) {
	  detail::impl_removeComponent<entity_t, Components...>{}(entity, component_id);
  }
...
struct impl_removeComponent<Entity>{
...
template < typename Component>
struct impl_getComponentNumber<Component> {
	  uint32_t n;
	  impl_getComponentNumber(uint32_t n) : n(n) {}
