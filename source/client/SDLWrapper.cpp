...
{
	if (!surface_)
		throw std::runtime_error("Cannot load image '" + filename + "': " + std::string(IMG_GetError()));
}
...
{
	if (!font_)
		throw std::runtime_error("Cannot open font file " + file + ": " + std::string(TTF_GetError()));
}
...
MySDL::Surface MySDL::Font::renderText_solid(std::string const &text, Rgb color) {
	return {TTF_RenderText_Blended(font_, text.c_str(), {color.r, color.g, color.b, 255})};
}
